...
export default function LivePage() {
  ...
  const [leaderboard, setLeaderboard] = useState<
    Array<{ username: string; score: number; profileUrl: string }>
  >([]);
  ...
  function connect(e: React.FormEvent<HTMLFormElement>) {
    ...
    function handlePackets(packet: ServerPeerPacket) {
      if (packet.type === ClientPacketTypes.Screen) {
        ...
        //@ts-expect-error we know that packet.data is an object
        const leaderboard = packet.data.leaderboard as Array<{
          username: string;
          score: number;
          profileUrl: string;
        }>;
  ...
  if (!connection) {
    return (
      <div>
        <h1 className={"text-3xl mb-2.5 font-bold text-center"}>
          Connect to a game
        </h1>
        <form
          onSubmit={connect}
        ...
        >
          <input
            maxLength={10}
            className={
              "mb-1 p-1 bg-gray-100 rounded dark:bg-gray-700 dark:text-white"
            }
            type="text"
          ...
          />
          <button
            aria-disabled={connecting || !ready}
            disabled={connecting || !ready}
            className={`text-black bg-gray-100 dark:bg-gray-800 dark:text-white p-2.5 rounded ${connecting || !ready ? "animate-pulse" : ""}`}
            type="submit"
          >
      ...
      <div className="flex flex-row justify-between items-center bg-gray-100 dark:bg-gray-800 dark:text-white rounded p-1 mb-2.5">
        <div className="flex flex-row justify-start items-center gap-2.5">
          <Image
            ...
            alt={usernameRef.current?.value || "User"}
            width={50}
            height={50}
          />
          <h1 className="text-2xl font-bold">
            {connection?.metadata.username || "User"}
          </h1>
        </div>
        <h1>
...
function ResultsScreen({
  ...
  leaderboard: Array<{ username: string; score: number; profileUrl: string }>;
}) {
